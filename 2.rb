...
def parse_game(game)
  ...
  game_id = game_id.scan(/\d+/).first.to_i
  sets = cubes.split(';').map { |set| parse_set(set) }
  [game_id, sets]
end

def parse_set(set)
  set.split(',').map(&:strip).map do |cube|
...
def valid_game?(game_sets, limitations) = game_sets.all? { valid_set?(_1, limitations) }
def valid_set?(set, limitations) = set.all? { |color, count| limitations.fetch(color) >= count }

valid_game_ids = input.filter_map do |game|
  game_id, game_sets = parse_game(game)
  next unless valid_game?(game_sets, LIMITATIONS)

  game_id
end

pp "Parb a: #{valid_game_ids.sum}"

# Part b
cube_power = input.sum do |game|
